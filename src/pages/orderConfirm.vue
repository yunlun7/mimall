...
export default{
  ...
  data () {
    return {
      ...
      count: 0, // 商品结算数量
      checkItem: {}, // 选中的商品的对象
      userAction: '', // 用户的操作: 0 添加  1 编辑  2 删除
  ...
  methods: {
    ...
    submitAddress () {
      ...
      if (userAction === 0 || userAction === 1) {
        // 通过解构的方式进行校验
        let {receiverName, receiverMobile, receiverProvince, receiverCity, receiverAddress, receiverDistrict, receiverZip} = checkItem
        let errMsg = ''
        if (!receiverName) {
          errMsg = '请输入收货人名称'
    ...
    getCartList () {
      this.axios.get('/carts').then((res) => {
        ...
        this.cartTotalPrice = res.cartTotalPrice // 获取商品总金额
